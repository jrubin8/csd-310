...
def show_menu():    
    print("\nWhatABook Main Menu:\n")
    print("1. View available books\n2. Browse WhatABook store locations\n3. My WhatABook account\n4. Exit this application\n\n")    
    try:
        choice = int(input('For example, enter 2 to view WhatABook locations: '))
        return choice
    except ValueError:        
        print("\n\nYour selection was invalid, so the program is being terminated. Please feel free to try again!\n")
        sys.exit(0)
#main menu^
...
def show_account_menu():
    try:        
        ...
        print("1. My Wishlist\n\n2. Add to My Wishlist\n\n3. Main Menu")        
        account_option = int(input('For example, enter 3 to go back to the Main Menu: '))        
        return account_option
    except ValueError:        
        print("\nYour entry was invalid, so the program is terminating. Please feel free to try again.\nGoodbye.\n")
        sys.exit(0)        
...
def validate_user():
    try:        
        ...
        if user_id < 0 or user_id > 3 or user_id == 0:            
            print("\nThat customer number wasn't valid. This program will terminate. Try again with a valid customer id.\n")
            sys.exit(0)
        return user_id
    except ValueError:        
        print("That wasn't a valid customer id. This program will now be terminated. Try again with a valid customer id.\n")
        sys.exit(0)        
#validates user_id^               
...
#lines 112 through 136 came from the course repository^
